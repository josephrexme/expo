...
- [iOS] `JavaScriptValuesBuffer` now holds its runtime as `unowned(unsafe)`, dropping the per-call unowned retain/release the buffer paid when copied or destroyed on the host-call hot path (measured ~4% faster on the no-op `@JS` host-call floor). The buffer is strictly call-scoped, so the runtime is always alive while it exists. ([#46949](https://github.com/expo/expo/pull/46949) by [@tsapeta](https://github.com/tsapeta))
