...
struct PendingErrorStoreTests {
  ...
  func `drains multiple errors oldest-first by timestamp`() throws {
    try withTemporaryDirectory { directory in
      PendingErrorStore.write(makePendingError(message: "first", sessionId: "s", timestamp: "2026-01-01T00:00:01Z"), in: directory)
      PendingErrorStore.write(makePendingError(message: "second", sessionId: "s", timestamp: "2026-01-01T00:00:02Z"), in: directory)
